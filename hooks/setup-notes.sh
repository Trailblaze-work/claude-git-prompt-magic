...
# Claude Code prompt notes (refs/notes/claude-prompt-trail).
#
...
if git remote get-url origin >/dev/null 2>&1; then
    ...
    if git config --local --get-all remote.origin.push 2>/dev/null | grep -qF "refs/notes/claude-prompt-trail"; then
        git config --unset-all --local remote.origin.push "refs/notes/claude-prompt-trail" 2>/dev/null || true
    fi
fi
